    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            for (var i = 0; i < sitios.length; i++) {
                ...
                // Buscamos el contenedor donde se crearan las atractivos-cards
                for (var j = 0; j < tarjetas[i].childNodes.length; j++) {
                    var nodoHijo = tarjetas[i].childNodes[j];
                    if (nodoHijo.id === "atractivos-cards") {
                        var contenedorAtractivos = nodoHijo.childNodes[1];
                    }
                }
